...
export class DetailedComplaintComponent implements OnInit {
  ...
  postComment() {
    ...
    this.dataService.FirePOST<IComplaintComment>(API.CommentPost, this.comment, { complaintId: this.detailedComplaint.id }).subscribe(result => {
      this.refresh();
      console.log(result);
      this.comment.text = '';
    });
  ...
  upVoteComplaint(has_upvoted: boolean) {
    ...
    if (has_upvoted === true)
      ...
      complaint_upvoted = 0;
    this.dataService.FireGET<IComplaint>(API.UpVote, { complaintId: this.detailedComplaint.id, action: complaint_upvoted === 0 ? 1 : 0 }).subscribe(result => {
      this.refresh();
      console.log(result);
  ...
  subscribe(complaint_subscribed: number) {
    this.dataService.FireGET<IComplaint>(API.SubscribeToComplaint, { complaintId: this.detailedComplaint.id, action: complaint_subscribed === 0 ? 1 : 0 }).subscribe(result => {
      this.refresh();
      console.log(result);
