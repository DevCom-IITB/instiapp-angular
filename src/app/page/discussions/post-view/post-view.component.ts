...
export class PostViewComponent implements OnInit {
  ...
  refresh(): void {
    this.dataService.fillGetPost(this.post_id).subscribe(result => {
      this.post = result;
    });
  }
  ...
  populateDummyPost() {
    ...
    const dummy_content_size = this.dummy_text.length;
    ...
    this.post = {
        ...
        time_of_creation: new Date(2010 + Math.floor(Math.random() * 20), 
        1 + Math.floor(Math.random() * 12), 1 + Math.floor(Math.random() * 28)),
      } as ICommunityPost;
